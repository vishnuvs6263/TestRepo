...
public class Staff_CreateEvent extends BaseClass {
	public static WebDriverWait wait;
	public static Actions actions;
	public static Select select;

	@Test
	//New features scripting- Student staff allocation - negative scenarios
	public void staff_createevent() throws Exception {
		...
		select = new Select(objects.selectSStudentDrpDwn());
		select.selectByVisibleText(prop.getProperty("Student_SupportDetails"));
		...
		//String CurrMonthActual = CalMonthYear.substring(0, CalMonthYear.lastIndexOf(" "));
			...
			if (windowTitle.contentEquals("Create Session")) {
				...
				objects.EventTitle().sendKeys(prop.getProperty("EventTitleStaff"));
				...
				if (actualStudentName.contentEquals(prop.getProperty("Student_Fullname"))) {
					...
					System.out.println(
							"Verify studemt name for created event executed and passed successfully!!!  Staff Name is :  "
									+ actualStudentName);
				}
				...
				if (actualEventTitle.contentEquals(prop.getProperty("EventTitleStaff"))) {
					Assert.assertEquals(actualEventTitle, prop.getProperty("EventTitleStaff"));
					System.out.println(
							"Verify staff create event executed and passed successfully!!!  Staff Name is :  "
									+ actualEventTitle);
				}
