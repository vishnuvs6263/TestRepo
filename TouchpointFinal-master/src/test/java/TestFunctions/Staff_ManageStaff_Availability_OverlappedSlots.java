...
public class Staff_ManageStaff_Availability_OverlappedSlots extends BaseClass {
	public static Actions actions;
	public static WebDriverWait wait;
	public static Select select;

	@Test
	public void StaffAvailability_OverlappedSLots() throws Exception {
		...
		objects.Staff_Support_StartDate_Field2().click();
		wait.until(ExpectedConditions.elementToBeClickable(objects.Staff_avail_startDate()));
		objects.Staff_avail_startDate().click();
		objects.Staff_Support_EndDate_Field2().click();
		objects.Calendar_NextMonthBtn().click();
		wait.until(ExpectedConditions.elementToBeClickable(objects.Staff_avail_endDate()));
		objects.Staff_avail_endDate().click();
		Thread.sleep(1000);
		select  = new Select(objects.Staff_avail_repeatType2());
		select.selectByVisibleText("Monthly");
		Thread.sleep(3000);
		//wait.until(ExpectedConditions.elementToBeClickable(objects.Timing_Weekly_Monday()));
		//objects.Timing_Weekly_Monday().click();
		//objects.Timing_Weekly_Tuesday().click();
		objects.Staff_avail_saveBtn2().click();
		wait.until(ExpectedConditions.elementToBeClickable(objects.AvailabilityErrorMsg()));
		String ErrorMessage = objects.AvailabilityErrorMsg().getText();
		if (ErrorMessage.contentEquals("*Overlapped slots")) {
			Assert.assertEquals(ErrorMessage, "*Overlapped slots");
			System.out.println("Verify duplicate slots monthly executed and passed successfully!!!   "+ErrorMessage);
		}else {
			Assert.fail();
		}
